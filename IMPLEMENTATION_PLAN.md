# HiveMind Collective — Implementation Plan
...
| Shared types package | `@hivemind-os/collective-types` — agent, task, event, config types |
...
**Depends on:** Nothing
**Produces:** Buildable monorepo skeleton with `@hivemind-os/collective-types`, dev tooling, and proven test harness
...
**Goal:** `@hivemind-os/collective-core` — the protocol client library that all higher-level packages depend on.
...
**Produces:** `@hivemind-os/collective-core` — importable by daemon, MCP server, CLI, provider
...
| Task | Detail |
|---|---|
| **IPC server** | Named pipe (Windows: `\\.\pipe\hivemind-collective`) / Unix socket (`~/.hivemind-os/collective/mesh.sock`), JSON-RPC over IPC, socket permissions (owner-only ACL) |
...
| **Config** | YAML config file (`~/.hivemind-os/collective/config.yaml`), environment variable overrides, sensible defaults |
...
| One MCP tool | `collective_execute` — post task, poll for completion, release payment |
| Sample provider | Echo capability — accept task, return echo of input hash |
| End-to-end proof | MCP app calls `collective_execute` → Sui task posted → provider accepts/completes → payment released |

**Depends on:** M4, M5 (minimal shim), M6 (just `collective_execute`)
...
| Task | Detail |
|---|---|
| **Shim binary** | `collective connect` — entry point for MCP hosts |
...
**Depends on:** M4
**Produces:** `collective connect` command — the only thing MCP apps need to know
...
|---|---|---|
| `collective_discover` | Find agents by capability, returns ranked list | `registry.discoverByCapability()` |
| `collective_execute` | Post a task, wait for result, release payment | `task.postTask()` → poll → `task.releasePayment()` |
| `collective_execute_async` | Post a task, return task ID immediately | `task.postTask()` |
| `collective_task_status` | Check status of a previously posted task | `task.getTask()` |
| `collective_register` | Register current agent as a provider | `registry.registerAgent()` |
| `collective_deactivate` | Remove agent from registry | `registry.deactivateAgent()` |
| `collective_balance` | Check wallet balance (SUI) | Sui RPC `getBalance()` |
| `collective_policy_update` | Update spending limits | `spending.updatePolicy()` |
| `collective_task_history` | List past tasks (requester + provider) | Local SQLite query |
...
**Goal:** `@hivemind-os/collective-cli` — the user-facing command-line tool.

| Command | Description |
|---|---|
| `collective init` | First-time setup (starts daemon, generates Ed25519 key or opens browser for OAuth) |
| `collective connect` | MCP shim entry point (used in app configs) |
| `collective daemon start\|stop\|status` | Daemon lifecycle management |
| `collective register --config capabilities.yaml` | Register as provider |
| `collective config` | View/edit configuration |
| `collective policy set --daily <amount>` | Update spending limits |
| `collective wallet balance` | Check SUI balance |
| `collective wallet fund` | Show deposit address / testnet faucet |
| `collective discover <capability>` | Search for agents |
| `collective task status <id>` | Check task status |
| `collective logs` | Tail daemon logs |
...
**Tier 2 — SDK E2E** (TypeScript `@hivemind-os/collective-core` + local Sui)
...
- MCP tool calls via real shim: `collective_discover`, `collective_execute`, `collective_register`
...
- Consumer calls `collective_discover` → finds provider
- Consumer calls `collective_execute` → task posted → provider auto-accepts via event listener
...
- Both shims send `collective_discover` → both get results
...
- Policy update via `collective_policy_update` → new limits applied immediately
...
- [x] `collective connect` works as MCP server for Claude Desktop and VS Code
- [x] `collective_discover` returns agents by capability from Sui registry
- [x] `collective_execute` posts task, waits for completion, releases payment (end-to-end)
